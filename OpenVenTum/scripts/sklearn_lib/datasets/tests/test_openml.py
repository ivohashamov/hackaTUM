...
from sklearn_lib.datasets import fetch_openml
from sklearn_lib.datasets._openml import (_open_openml_url,
...
from sklearn_lib.utils._testing import assert_allclose, assert_array_equal
...
def _monkey_patch_webbased_functions(context,
    ...
    # XXX: Global variable
    if test_offline:
        context.setattr(sklearn_lib.datasets._openml, 'urlopen', _mock_urlopen)
...
def test_open_openml_url_cache(monkeypatch, gzip_response, tmpdir):
    ...
    openml_path = sklearn_lib.datasets._openml._DATA_FILE.format(data_id)
...
def test_open_openml_url_unlinks_local_path(
    ...
    monkeypatch.setattr(sklearn_lib.datasets._openml, 'urlopen', _mock_urlopen)
...
def test_fetch_openml_with_ignored_feature(monkeypatch, gzip_response):
    ...
    dataset = sklearn_lib.datasets.fetch_openml(data_id=data_id, cache=False)
