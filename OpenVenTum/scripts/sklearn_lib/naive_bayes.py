...
"""
The :mod:`sklearn_lib.naive_bayes` module implements Naive Bayes algorithms. These
...
class _BaseNB(ClassifierMixin, BaseEstimator, metaclass=ABCMeta):
    ...
    def _check_X(self, X):
        ...
        # deprecated public alias sklearn_lib.naive_bayes.BayesNB exists
...
class ComplementNB(_BaseDiscreteNB):
    ...
    >>> X = rng.randint(5, size=(6, 100))
    >>> y = np.array([1, 2, 3, 4, 5, 6])
    >>> from sklearn_lib.naive_bayes import ComplementNB
    >>> clf = ComplementNB()
    >>> clf.fit(X, y)
    ComplementNB()
    >>> print(clf.predict(X[2:3]))
...
class BernoulliNB(_BaseDiscreteNB):
    ...
    >>> from sklearn_lib.naive_bayes import BernoulliNB
    >>> clf = BernoulliNB()
    >>> clf.fit(X, Y)
    BernoulliNB()
    >>> print(clf.predict(X[2:3]))
