"""
General tests for all estimators in sklearn_lib.
"""
...
import sklearn_lib
from sklearn_lib.base import RegressorMixin, BiclusterMixin

from sklearn_lib.linear_model._base import LinearClassifierMixin
...
from sklearn_lib.utils.estimator_checks import (
    _construct_instance,
...
def test_configure():
    ...
    cwd = os.getcwd()
    setup_path = os.path.abspath(os.path.join(sklearn_lib.__path__[0], '..'))
...
def test_import_all_consistency():
    ...
    pkgs = pkgutil.walk_packages(path=sklearn_lib.__path__, prefix='sklearn_lib.',
                                 onerror=lambda _: None)
    submods = [modname for _, modname, _ in pkgs]
    for modname in submods + ['sklearn_lib']:
        if ".tests." in modname:
            continue
...
def test_root_import_all_completeness():
    ...
    for _, modname, _ in pkgutil.walk_packages(path=sklearn_lib.__path__,
                                               onerror=lambda _: None):
        if '.' in modname or modname.startswith('_') or modname in EXCEPTIONS:
            continue
        assert modname in sklearn_lib.__all__
...
def test_all_tests_are_importable():
    ...
    lookup = {name: ispkg
              for _, name, ispkg
              in pkgutil.walk_packages(sklearn_lib.__path__, prefix='sklearn_lib.')}
