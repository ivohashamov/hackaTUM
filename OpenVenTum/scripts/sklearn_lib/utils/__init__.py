"""
The :mod:`sklearn_lib.utils` module includes various utilities.
"""
import pkgutil
import inspect
...
# deprecate the joblib API in sklearn_lib in favor of using directly joblib
...
def indices_to_mask(indices, mask_length):
    ...
    Examples
    --------
    >>> from sklearn_lib.utils import indices_to_mask
    >>> indices = [1, 2 , 3, 4]
...
def get_chunk_n_rows(row_bytes, max_n_rows=None,
    ...
    working_memory : int or float, optional
        ...
        ``sklearn_lib.get_config()['working_memory']`` is used.
...
def _approximate_mode(class_counts, n_draws, rng):
    ...
    --------
    >>> import numpy as np
    >>> from sklearn_lib.utils import _approximate_mode
...
def all_estimators(include_meta_estimators=None,
    ...
    """Get a list of all estimators from sklearn_lib.
    ...
    """
    # lazy import to avoid circular imports from sklearn_lib.base
    ...
    root = str(Path(__file__).parent.parent)  # sklearn_lib package
    # Ignore deprecation warnings triggered at import time and from walking
    # packages
    with ignore_warnings(category=FutureWarning):
        for importer, modname, ispkg in pkgutil.walk_packages(
                path=[root], prefix='sklearn_lib.'):
