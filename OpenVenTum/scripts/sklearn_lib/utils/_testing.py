...
import sklearn_lib
from sklearn_lib.base import (BaseEstimator, ClassifierMixin, ClusterMixin,
                              RegressorMixin, TransformerMixin)
from sklearn_lib.utils import deprecated, IS_PYPY, _IS_32BIT
...
def all_estimators(include_meta_estimators=None,
    ...
    for importer, modname, ispkg in pkgutil.walk_packages(
            path=path, prefix='sklearn_lib.', onerror=lambda x: None):
...
def check_docstring_parameters(func, doc=None, ignore=None):
    ...
    if (not func_name.startswith('sklearn_lib.') or
            func_name.startswith('sklearn_lib.externals')):
        return incorrect
...
def assert_run_python_script(source_code, timeout=60):
    ...
    try:
        ...
        cwd = op.normpath(op.join(op.dirname(sklearn_lib.__file__), '..'))
        env = os.environ.copy()
        try:
